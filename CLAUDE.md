...
## ГЛАВНОЕ ПРАВИЛО
**Прогресс необратим.** Никогда не откатывать параметры, настройки, интеграции и код назад.
Всё что сделано — текущее состояние. Двигаемся только вперёд.
...
## Авто-брифинг — ЗАПУСКАТЬ АВТОМАТИЧЕСКИ ПРИ СТАРТЕ КАЖДОЙ СЕССИИ

**ОБЯЗАТЕЛЬНО:** При любом первом сообщении в новой сессии — сначала выдай брифинг по всем 4 проектам, затем отвечай на вопрос пользователя.

### Проекты владельца (всегда 4):
1. 💎 **Amberra** — amberrajewelry.com (ювелирный e-commerce)
2. ✈️ **Сахватрэвл** — travel проект
3. 🤖 **Крипто-бот** — crypto bot
4. ⚖️ **Спектр Права** — юридический проект

### ☀️ Структура авто-брифинга при старте

**БЛОК 1 — ДАННЫЕ (запрашивать автоматически):**
- GSC Amberra → Supermetrics `ds_id=GW`, аккаунт `https://www.amberrajewelry.com/`, метрики за вчера: клики, показы, CTR, позиция
- Ошибки → Sentry DSN `4685202527800b7a5362a10c52b9ba1b`

**БЛОК 2 — СТАТУС ПРОЕКТОВ (из памяти):**
Читай файл `/Users/vladimir/.claude/projects/-Users-vladimir/memory/daily_tasks.md`
Для каждого из 4 проектов показать: текущий статус + топ-задачи

**БЛОК 3 — ПЛАН ДНЯ:**
- 3 приоритетных задачи на сегодня (по всем проектам)
- 💡 Одна конкретная рекомендация

Формат: компактно, по проектам, без воды. Недоступные данные — пометить ⚠️ и продолжить.

---

### 🌙 Команда `/вечер`

1. Итоги дня по всем 4 проектам: что сделано
2. Предложить 3–5 задач на завтра по каждому проекту
3. Сохранить выбор в `/Users/vladimir/.claude/projects/-Users-vladimir/memory/daily_tasks.md`

### Команды
- `/утро` — полный брифинг вручную
- `/вечер` — итоги дня + план на завтра
- `/gsc` — только GSC данные Amberra
- `/задачи` — активные задачи по всем проектам
- `/amberra` — только статус Amberra
- `/все` — детальный отчёт по всем 4 проектам
