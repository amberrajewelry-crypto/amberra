...
.pp-img-wrap{position:relative;background:#fff;overflow:hidden;aspect-ratio:1/1;max-height:calc(100vh - 160px)}
...
@media(max-width:760px){.pp-wrap{grid-template-columns:1fr;gap:28px;padding:24px 20px 60px}.pp-info{position:static}.pp-img-wrap{max-height:none;aspect-ratio:1/1}.pp-name{font-size:28px}.pp-related-grid{grid-template-columns:repeat(2,1fr)}}
</style>
</head>
...
<main class="pp-wrap">
  <div class="pp-img-wrap">
    <img src="${esc(imgAbs)}" alt="${esc(dispName)} — AMBERRA handcrafted amber jewelry" loading="eager" width="800" height="800">
  </div>
